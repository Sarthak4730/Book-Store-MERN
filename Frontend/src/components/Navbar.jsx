...
const Navbar = ( { count } ) => {
  ...
  return(
    <nav className="flex justify-between items-center h-[7.5vh] w-[100vw] fixed top-0 px-10 bg-white z-10">
        <div className="left flex w-[23vw] justify-between">
            <Link to="/">
            ...
            <div className="searchbar-div mt-1 bg-gray-300 w-[20vw] flex justify-between p-[5px] rounded-md">
              ...
              <input className="w-[17vw] focus:outline-none" type="text" name="searchbar" id="searchbar" placeholder="What are you looking for?"/>
            </div>
        </div>

        <div className={`right flex ${ currentUser ? 'w-[17.5vw]' : 'w-[10vw]' } justify-between`}>
          <Link to="/login">
            { currentUser 
              ? <img className="hover:scale-115 w-[30px] h-[30px] rounded-full" src={currentUser.photoURL} alt="pfp" />
